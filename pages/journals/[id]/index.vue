<template>
  <div class="books-banner">
    ...
    <div class="container">
      <ul class="books-banner__nav">
        ...
        <li>
          <NuxtLink :to="localePath('/')">{{ $t("ScientificJournals") }}</NuxtLink>
        </li>
      </ul>
    </div>
  </div>
  <div class="journals">
    <div class="container">
      ...
      <div class="journals-wrapper" ref="scrollComponent">
        <div
        ...
        >
          <div class="p-6">
            <div class="flex items-left flex-col gap-2 justify-between mb-3">
              <NuxtLink
                :to="localePath(`/journals/${item?.category}/${item?.slug}`)"
              ...
              <p
                class="text-sm whitespace-nowrap flex items-center flex-wrap gap-2"
              >
                <span v-for="item in item?.authors" :key="item">{{
                  item?.full_name
                }}</span>
              </p>
            </div>
            <p
              class="block font-sans mb-6 text-base antialiased font-light leading-relaxed"
...
const lastSizeCount = ref(0);
async function getAllArticles() {
  const res = await Service.getCategoryArticle(
    id,
    locale.value,
    sizeCount.value
  );
  articles.value = res.data;
  lastSizeCount.value = res.data.count;
}
getAllArticles();
...
async function filter() {
  const res = await Service.articleFilter(
    id,
    search.value,
    start.value,
    end.value,
...
const handleScroll = (e) => {
  ...
  if (sizeCount.value < lastSizeCount.value) {
